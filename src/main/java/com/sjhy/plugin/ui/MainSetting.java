...
public class MainSetting implements Configurable, Configurable.Composite {
  ...
  public MainSetting() {
    ...
    resetBtn.addActionListener(e -> {
      if (MessageDialogBuilder.yesNo(MsgValue.TITLE_INFO, MsgValue.RESET_DEFAULT_SETTING_MSG)
          .isYes()) {
    ...
    importBtn.addActionListener(e -> {
      String token = Messages
          .showInputDialog("Token:", MsgValue.TITLE_INFO, AllIcons.General.PasswordLock, "",
              new InputValidator() {
                @Override
      ...
      try {
        ...
        // 配置覆盖
        coverConfig(jsonNode, StrState.TYPE_MAPPER, TypeMapperGroup.class,
            settings.getTypeMapperGroupMap());
        coverConfig(jsonNode, StrState.SERVICE_MODULE, ServiceModuleGroup.class,
            settings.getServiceModuleGroupMap());
        coverConfig(jsonNode, StrState.TEMPLATE, TemplateGroup.class,
            settings.getTemplateGroupMap());
        coverConfig(jsonNode, StrState.COLUMN_CONFIG, ColumnConfigGroup.class,
            settings.getColumnConfigGroupMap());
        coverConfig(jsonNode, StrState.GLOBAL_CONFIG, GlobalConfigGroup.class,
            settings.getGlobalConfigGroupMap());
        // 重置配置
    ...
    exportBtn.addActionListener(e -> {
      ...
      ListCheckboxPanel typeMapperPanel = new ListCheckboxPanel("Type Mapper",
          settings.getTypeMapperGroupMap().keySet());
      mainPanel.add(typeMapperPanel);
      // Service module
      ListCheckboxPanel serviceModulePanel = new ListCheckboxPanel("Service Module",
          settings.getServiceModuleGroupMap().keySet());
      mainPanel.add(serviceModulePanel);
      // Template
      ListCheckboxPanel templatePanel = new ListCheckboxPanel("Template",
          settings.getTemplateGroupMap().keySet());
      mainPanel.add(templatePanel);
      // Column Config
      ListCheckboxPanel columnConfigPanel = new ListCheckboxPanel("Column Config",
          settings.getColumnConfigGroupMap().keySet());
      mainPanel.add(columnConfigPanel);
      // GlobalConfig
      ListCheckboxPanel globalConfigPanel = new ListCheckboxPanel("Global Config",
          settings.getGlobalConfigGroupMap().keySet());
      ...
      dialogBuilder.addActionDescriptor(dialogWrapper -> new AbstractAction("OK") {
        ...
        public void actionPerformed(ActionEvent e) {
          if (!MainSetting.this
              .isSelected(typeMapperPanel, serviceModulePanel, templatePanel, columnConfigPanel, globalConfigPanel)) {
          ...
          // 打包用户模块数据
          Map<String, ServiceModuleGroup> serviceModule = new LinkedHashMap<>();
          for (String selectedItem : serviceModulePanel.getSelectedItems()) {
            serviceModule.put(selectedItem, settings.getServiceModuleGroupMap().get(selectedItem));
          }
          param.put(StrState.SERVICE_MODULE, serviceModule);
   ...
   * @param srcGroup 源分组
   */
  private <T extends AbstractGroup> void coverConfig(@NotNull JsonNode jsonNode,
      @NotNull String name, Class<T> cls, @NotNull Map<String, T> srcGroup) {
    ObjectMapper objectMapper = new ObjectMapper();
      ...
      while (names.hasNext()) {
        ...
        if (srcGroup.containsKey(key)) {
          if (!MessageDialogBuilder
              .yesNo(MsgValue.TITLE_INFO, String.format("是否覆盖%s配置中的%s分组？", name, key)).isYes()) {
            continue;
          }
        }
  ...
  public Configurable[] getConfigurables() {
    ...
    result[1] = new ServiceModuleSetting(settings);
    result[2] = new TemplateSettingPanel();
  ...
  public boolean isModified() {
    return !settings.getEncode().equals(encodeComboBox.getSelectedItem()) || !settings.getAuthor()
        .equals(authorTextField.getText());
  }
